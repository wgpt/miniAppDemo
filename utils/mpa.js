    ...
    r(op).then((data) => {
        ...
        if (data == 401) { // 未授权进入队列
            ...
            op.errorShow && wx.$.showToast({
                title: data.message || '获取信息错误（错误码：4000）',
                icon: 'none'
            })
    ...
    return new Promise((resolve, reject) => {
        ...
        wx.request({
            ...
            success(e) {
                ...
                if (e.data.code == 'not_found_user') { // token 有误
...
function auth(op, reGet = true) { // 授权
    ...
    wx.login({
        success(res) {
            wx.request({
                url: wx.config.URL + '/auth/wx_login',
                data: {
                    code: res.code,
                },
                method: 'post',
                ...
                success(json) {
                    ...
                    if (json.data.code == 'not_found_user') { // 新用户
...
function clearApiList(onlySetNull = false) { // 清空队列,重新回调
